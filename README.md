...
**Web app (Flask, in your browser)**
```bash
python app.py
```
Then open http://127.0.0.1:5000 in your browser. The page shows the live
annotated feed, current EAR, a status banner, and plays an alert sound in
the browser when drowsiness is detected. Stop it with Ctrl+C in the terminal.

Both entry points share the same detection logic in `detector.py` — no
duplicated code between the CLI and web versions.

## Project structure
```
drowsiness-detection/
├── detector.py         # shared EAR / face-landmark detection logic
├── main.py             # CLI entry point (OpenCV window)
├── app.py              # Flask entry point (browser)
├── download_model.py   # one-time model download script
├── templates/
│   └── index.html      # web app page
├── static/
│   ├── style.css
│   └── alert.wav        # browser alert sound
├── requirements.txt
└── README.md
```
